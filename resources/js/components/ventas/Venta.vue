            ...
            <div class="row">
              <div class="col-md-12">
                ...
                <template v-if="venta.tipo_venta == 2">
                  <p class="col-md-6 px-0 mb-1 text-muted"> Monto de las cuotas: {{ montoCuotas | currency }} </p>
                  <p class="col-md-6 px-0 mb-1 text-muted"> Número de cuotas: <span style="font-weight:600;">{{ venta.acuerdo_pago.cuotas  }} </span>  </p>                  
                  <p class="col-md-6 px-0 mb-1 text-muted"> Abono: <span style="font-weight:600;">{{ venta.abono | currency  }} </span>  </p>                  
                  <p class="col-md-6 px-0 mb-1 text-muted"> Descuento: <span style="font-weight:600;">{{ venta.Descuento | currency  }} </span>  </p>                  
                  <p class="col-md-7 px-0 mb-1 text-muted"> Período de pago: <span style="font-weight:600;">{{ venta.acuerdo_pago.periodo_pago}} </span> </p>
                </template>  
              </div>
            </div>
            <div class="row">
              <div class="col-md-12">
                <p style="font-size:16px;" class="mt-2 mb-1">Productos vendidos</p>
                <table class="table table-condensed">
                  <tr v-for='item in venta.productos_venta' :key="item.id+'a'"> 
                    <td>{{ item.producto }} <b>x{{ item.cantidad }}</b> </td>  
                  </tr>
                  <tr>
                    <th style="font-size:16px;padding-left:10px"> Sub-total: {{ venta.subtotal | currency }}</th>
                  </tr>
                  <tr>
                    <th style="font-size:18px; padding-top:10px"> Total: {{ venta.total | currency }}</th>
                  </tr>
                </table>
              </div>
            </div>
